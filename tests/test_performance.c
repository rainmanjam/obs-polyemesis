...
static bool bench_api_client_lifecycle(perf_metrics_t *metrics) {
  ...
  for (size_t i = 0; i < iterations; i++) {
    restreamer_connection_t conn = {.host = "localhost",
                                    .port = 8080,
                                    .username = NULL,
                                    .password = NULL,
                                    .use_https = false};
    ...
    if (!api) {
      fprintf(stderr, "Failed to create API client at iteration %zu\n", i);
      return false;
    }
...
static bool bench_multistream_config(perf_metrics_t *metrics) {
  ...
  for (size_t i = 0; i < iterations; i++) {
    // Add destinations
    restreamer_multistream_add_destination(config, SERVICE_TWITCH, "test_key_1",
                                           ORIENTATION_HORIZONTAL);
    restreamer_multistream_add_destination(
        config, SERVICE_YOUTUBE, "test_key_2", ORIENTATION_HORIZONTAL);
    restreamer_multistream_add_destination(config, SERVICE_TIKTOK, "test_key_3",
...
static bool bench_orientation_detection(perf_metrics_t *metrics) {
  ...
  for (size_t i = 0; i < iterations; i++) {
    ...
    result = restreamer_multistream_detect_orientation(1920, 1080); // 16:9
    result = restreamer_multistream_detect_orientation(1080,
                                                       1920); // 9:16
    result = restreamer_multistream_detect_orientation(1080,
                                                       1080);       // 1:1
    result = restreamer_multistream_detect_orientation(2560, 1440); // 16:9
