...
restreamer_api_t *restreamer_api_create(restreamer_connection_t *connection)
{
	...
	/* Set up headers */
	api->headers = curl_slist_append(api->headers,
				  ...
				  restreamer_process_list_t *list)
{
	...
	if (!root) {
		dstr_printf(&api->last_error, "JSON parse error: %s",
			    error.text);
		return false;
	}
	...
	for (size_t i = 0; i < count; i++) {
		...
		if (json_is_string(reference)) {
			process->reference =
				bstrdup(json_string_value(reference));
		}
		...
		if (json_is_string(command)) {
			process->command =
				bstrdup(json_string_value(command));
		}
	}
...
bool restreamer_api_start_process(restreamer_api_t *api, const char *process_id)
{
	if (!api || !process_id) {
		return false;
	}
	...
	struct memory_struct response;
	bool result =
		make_request(api, endpoint.array, "POST", post_data, &response);
				     ...
				     restreamer_log_list_t *logs)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		if (json_is_string(timestamp)) {
			entry->timestamp =
				bstrdup(json_string_value(timestamp));
		}
				 ...
				 restreamer_session_list_t *sessions)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		if (json_is_string(session_id)) {
			session->session_id =
				bstrdup(json_string_value(session_id));
		}
		...
		json_t *bytes_sent =
			json_object_get(session_obj, "bytes_sent");
		if (json_is_integer(bytes_sent)) {
			session->bytes_sent = json_integer_value(bytes_sent);
		}

		json_t *bytes_received =
			json_object_get(session_obj, "bytes_received");
		if (json_is_integer(bytes_received)) {
			session->bytes_received =
				json_integer_value(bytes_received);
		}

		json_t *remote_addr =
			json_object_get(session_obj, "remote_addr");
		if (json_is_string(remote_addr)) {
			session->remote_addr =
				bstrdup(json_string_value(remote_addr));
		}
	}
				   ...
				   const char *process_id)
{
	...
	struct memory_struct response;
	bool result =
		make_request(api, endpoint.array, "DELETE", NULL, &response);
	dstr_free(&endpoint);
