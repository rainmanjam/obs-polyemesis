...
bool OBSServiceLoader::loadServices() {
...
#ifdef __APPLE__
  possiblePaths << "/Applications/OBS.app/Contents/PlugIns/rtmp-services.plugin/Contents/Resources/services.json";
#endif

  // 2. User config directory (custom services)
  QString userConfig = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
  ...
  possiblePaths << "/usr/local/share/obs/obs-plugins/rtmp-services/services.json";
#endif

  // 4. Windows locations
#ifdef _WIN32
  QString programFiles = qEnvironmentVariable("ProgramFiles");
  possiblePaths << programFiles + "/obs-studio/data/obs-plugins/rtmp-services/services.json";
#endif
  ...
  obs_log(LOG_WARNING, "[OBS Service Loader] Could not find services.json in any expected location");
  return false;
}
...
bool OBSServiceLoader::tryLoadFromPath(const QString& path) {
  ...
  if (!root) {
    obs_log(LOG_WARNING, "[OBS Service Loader] Failed to parse JSON: %s", error.text);
    return false;
  }
  ...
  if (codecsArray && json_is_array(codecsArray)) {
    ...
    json_array_foreach(codecsArray, index, codecObj) {
      if (json_is_string(codecObj)) {
        service.supported_video_codecs.append(QString::fromUtf8(json_string_value(codecObj)));
      }
    }
  }
...
const StreamingService* OBSServiceLoader::getService(const QString& name) const {
