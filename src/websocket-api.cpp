...
static void handle_create_profile(obs_data_t *request, obs_data_t *response,
                                  void *priv);
static void handle_delete_profile(obs_data_t *request, obs_data_t *response,
                                  void *priv);
static void handle_duplicate_profile(obs_data_t *request, obs_data_t *response,
                                     void *priv);
static void handle_get_profiles(obs_data_t *request, obs_data_t *response,
                                void *priv);
static void handle_add_destination(obs_data_t *request, obs_data_t *response,
                                   void *priv);
static void handle_remove_destination(obs_data_t *request, obs_data_t *response,
                                      void *priv);
static void handle_edit_destination(obs_data_t *request, obs_data_t *response,
                                    void *priv);
static void handle_start_profile(obs_data_t *request, obs_data_t *response,
                                 void *priv);
static void handle_stop_profile(obs_data_t *request, obs_data_t *response,
                                void *priv);
static void handle_start_all_profiles(obs_data_t *request, obs_data_t *response,
                                      void *priv);
static void handle_stop_all_profiles(obs_data_t *request, obs_data_t *response,
                                     void *priv);
static void handle_get_plugin_state(obs_data_t *request, obs_data_t *response,
                                    void *priv);
static void handle_get_profile_state(obs_data_t *request, obs_data_t *response,
                                     void *priv);
static void handle_get_connection_status(obs_data_t *request,
                                         obs_data_t *response, void *priv);
static void handle_get_button_states(obs_data_t *request, obs_data_t *response,
                                     void *priv);
static void handle_connect_to_server(obs_data_t *request, obs_data_t *response,
                                     void *priv);
...
bool websocket_api_init(void) {
  ...
  obs_websocket_vendor_register_request(vendor, "CreateProfile",
                                        handle_create_profile, nullptr);
  obs_websocket_vendor_register_request(vendor, "DeleteProfile",
                                        handle_delete_profile, nullptr);
  obs_websocket_vendor_register_request(vendor, "DuplicateProfile",
                                        handle_duplicate_profile, nullptr);
  obs_websocket_vendor_register_request(vendor, "GetProfiles",
                                        handle_get_profiles, nullptr);

  obs_websocket_vendor_register_request(vendor, "AddDestination",
                                        handle_add_destination, nullptr);
  obs_websocket_vendor_register_request(vendor, "RemoveDestination",
                                        handle_remove_destination, nullptr);
  obs_websocket_vendor_register_request(vendor, "EditDestination",
                                        handle_edit_destination, nullptr);

  obs_websocket_vendor_register_request(vendor, "StartProfile",
                                        handle_start_profile, nullptr);
  obs_websocket_vendor_register_request(vendor, "StopProfile",
                                        handle_stop_profile, nullptr);
  obs_websocket_vendor_register_request(vendor, "StartAllProfiles",
                                        handle_start_all_profiles, nullptr);
  obs_websocket_vendor_register_request(vendor, "StopAllProfiles",
                                        handle_stop_all_profiles, nullptr);

  obs_websocket_vendor_register_request(vendor, "GetPluginState",
                                        handle_get_plugin_state, nullptr);
  obs_websocket_vendor_register_request(vendor, "GetProfileState",
                                        handle_get_profile_state, nullptr);
  obs_websocket_vendor_register_request(vendor, "GetConnectionStatus",
                                        handle_get_connection_status, nullptr);
  obs_websocket_vendor_register_request(vendor, "GetButtonStates",
                                        handle_get_button_states, nullptr);
  obs_websocket_vendor_register_request(vendor, "ConnectToServer",
                                        handle_connect_to_server, nullptr);

  blog(LOG_INFO,
       "[obs-polyemesis] WebSocket Vendor API initialized with 16 requests");
  return true;
}
...
static void handle_delete_profile(obs_data_t *request, obs_data_t *response,
  ...
  if (profile->status != PROFILE_INACTIVE) {
    ...
    obs_data_set_string(response, "error",
                        "Cannot delete active profile - stop it first");
    return;
  }
  ...
  blog(LOG_INFO, "[obs-polyemesis] WebSocket: Deleted profile ID: %s",
       profile_id);
}
...
static void handle_duplicate_profile(obs_data_t *request, obs_data_t *response,
  ...
  output_profile_t *new_profile =
      profile_manager_duplicate_profile(pm, profile_id, new_name);
  if (!new_profile) {
    obs_data_set_bool(response, "success", false);
 ...
 * Request: {}
 * Response: {"profiles": [{"id": "uuid", "name": "Profile", "status": 0,
 * "destinationCount": 2}, ...]}
 */
static void handle_get_profiles(obs_data_t *request, obs_data_t *response,
                                void *priv) {
  ...
  for (size_t i = 0; i < pm->profile_count; i++) {
    ...
    obs_data_set_int(profile_obj, "destinationCount",
                     profile->destination_count);
    obs_data_set_string(profile_obj, "processId",
                        profile->process_id ? profile->process_id : "");
  ...
  blog(LOG_DEBUG, "[obs-polyemesis] WebSocket: Retrieved %zu profiles",
       pm->profile_count);
}

/**
 * AddDestination - Add streaming destination to profile
 * Request: {"profileId": "uuid", "name": "Twitch", "url": "rtmp://...",
 * "streamKey": "key"} Response: {"success": true, "destinationIndex": 0}
 */
static void handle_add_destination(obs_data_t *request, obs_data_t *response,
                                   void *priv) {
  UNUSED_PARAMETER(priv);
  ...
  if (!profile_id || !name || !url || !stream_key) {
    ...
    obs_data_set_string(
        response, "error",
        "Missing required fields: profileId, name, url, streamKey");
    return;
  }
  ...
  if (success) {
    obs_data_set_bool(response, "success", true);
    obs_data_set_int(response, "destinationIndex",
                     profile->destination_count - 1);
    blog(LOG_INFO,
         "[obs-polyemesis] WebSocket: Added destination '%s' to profile %s",
         name, profile_id);
  } else {
    obs_data_set_bool(response, "success", false);
...
static void handle_remove_destination(obs_data_t *request, obs_data_t *response,
  ...
  if (success) {
    blog(LOG_INFO,
         "[obs-polyemesis] WebSocket: Removed destination %d from profile %s",
         index, profile_id);
  }
}
 ...
 * Request: {"profileId": "uuid", "destinationIndex": 0, "name": "New Name",
 * "url": "...", "streamKey": "..."} Response: {"success": true}
 */
static void handle_edit_destination(obs_data_t *request, obs_data_t *response,
                                    void *priv) {
  UNUSED_PARAMETER(priv);
  ...
  if (!profile || index < 0 || index >= (int)profile->destination_count) {
    obs_data_set_bool(response, "success", false);
    obs_data_set_string(response, "error",
                        "Invalid profile or destination index");
    return;
  }
  ...
  if (success) {
    blog(LOG_INFO,
         "[obs-polyemesis] WebSocket: Edited destination %d in profile %s",
         index, profile_id);
  }
}
...
static void handle_start_profile(obs_data_t *request, obs_data_t *response,
  ...
  if (success) {
    ...
    blog(LOG_INFO, "[obs-polyemesis] WebSocket: Started profile %s",
         profile_id);
  } else {
...
static void handle_stop_profile(obs_data_t *request, obs_data_t *response,
  ...
  if (success) {
    ...
    blog(LOG_INFO, "[obs-polyemesis] WebSocket: Stopped profile %s",
         profile_id);
  } else {
...
static void handle_start_all_profiles(obs_data_t *request, obs_data_t *response,
  ...
  blog(LOG_INFO, "[obs-polyemesis] WebSocket: Started %d profiles",
       started_count);
}
...
static void handle_stop_all_profiles(obs_data_t *request, obs_data_t *response,
  ...
  for (size_t i = 0; i < pm->profile_count; i++) {
    ...
    if (profile->status == PROFILE_ACTIVE ||
        profile->status == PROFILE_STARTING) {
  ...
  blog(LOG_INFO, "[obs-polyemesis] WebSocket: Stopped %d profiles",
       stopped_count);
}
 ...
 * Request: {}
 * Response: {"connected": true, "serverUrl": "...", "profileCount": 5,
...
static void handle_get_plugin_state(obs_data_t *request, obs_data_t *response,
  ...
  obs_data_set_string(response, "serverUrl",
...
static void handle_get_profile_state(obs_data_t *request, obs_data_t *response,
  ...
  obs_data_set_string(response, "processId",
                      profile->process_id ? profile->process_id : "");
 ...
 * Request: {"profileId": "uuid"} (optional - if provided, checks buttons for
 * that profile) Response: {"canEdit": true, "canDuplicate": true, "canStart":
 * false, "canStop": true, ...}
 */
static void handle_get_button_states(obs_data_t *request, obs_data_t *response,
                                     void *priv) {
  ...
  output_profile_t *profile =
      has_selection ? profile_manager_get_profile_by_id(pm, profile_id)
                    : nullptr;

  bool is_active = profile && (profile->status == PROFILE_ACTIVE ||
                               profile->status == PROFILE_STARTING);
  ...
  obs_data_set_bool(response, "canStartProfile",
                    has_selection && is_inactive && has_destinations);
 ...
 * Request: {"serverUrl": "http://localhost:8080", "username": "admin",
 * "password": "admin"} Response: {"success": true}
 */
static void handle_connect_to_server(obs_data_t *request, obs_data_t *response,
                                     void *priv) {
  UNUSED_PARAMETER(priv);
  ...
  if (!server_url || !username || !password) {
    ...
    obs_data_set_string(
        response, "error",
        "Missing required fields: serverUrl, username, password");
    return;
  }
