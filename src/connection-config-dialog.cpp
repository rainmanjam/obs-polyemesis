...
void ConnectionConfigDialog::setupUI()
{
	...
	QLabel *urlHelpLabel = new QLabel(
		"<small style='color: #888;'>Tip: Include port number if not using standard ports (80/443)</small>");
...
void ConnectionConfigDialog::saveSettings()
{
	...
	if (!obs_data_save_json_safe(settings, config_path, "tmp", "bak")) {
		obs_log(LOG_ERROR, "Failed to save connection settings to %s",
			config_path);
		return;
	}
...
void ConnectionConfigDialog::onTestConnection()
{
	...
	obs_log(LOG_INFO,
		"Testing connection to: %s (username: %s, parsed from: %s)",
		connectionUrl.toUtf8().constData(),
		username.isEmpty() ? "(none)" : username.toUtf8().constData(),
		url.toUtf8().constData());

	/* Show testing status with connection details */
	m_statusLabel->setText(QString("🔄 Testing connection to %1...")
				       .arg(connectionUrl));
	m_statusLabel->setStyleSheet(
		"background-color: #1a3a5a; color: #6eb6ff; padding: 8px; border-radius: 4px;");
	m_statusLabel->show();
	...
	if (success) {
		...
		m_statusLabel->setStyleSheet(
			"background-color: #1a3a2a; color: #6eff6e; padding: 8px; border-radius: 4px;");
		...
		if (error_msg) {
			...
			if (errorStr.contains("401") ||
				...
				"   (Use port 443 for HTTPS with Let's Encrypt, or custom port like 8080)";
		...
		m_statusLabel->setStyleSheet(
			"background-color: #3a1a1a; color: #ff6e6e; padding: 8px; border-radius: 4px;");
...
void ConnectionConfigDialog::onSave()
{
	...
	emit settingsSaved(url, m_usernameEdit->text(),
			   m_passwordEdit->text(), m_timeoutSpinBox->value());

	accept();
}

void ConnectionConfigDialog::onCancel()
{
	reject();
}
