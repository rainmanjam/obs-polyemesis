...
static obs_service_t *create_rtmp_service(const char *url)
{
	...
	obs_service_t *service = obs_service_create("rtmp_custom", "polyemesis_rtmp_service", settings, NULL);
...
static obs_encoder_t *create_video_encoder(const char *name)
{
	...
	const char *encoder_id = "obs_x264";  /* Use software encoder for compatibility */
	...
	obs_data_set_int(encoder_settings, "bitrate", 6000);  /* 6 Mbps - good for 1080p */
	obs_data_set_string(encoder_settings, "preset", "veryfast");  /* Low CPU usage */
	...
	obs_encoder_t *encoder = obs_video_encoder_create(encoder_id, name, encoder_settings, NULL);
	obs_data_release(encoder_settings);
...
static obs_encoder_t *create_audio_encoder(const char *name)
{
	...
	obs_encoder_t *encoder = obs_audio_encoder_create(encoder_id, name, encoder_settings, 0, NULL);
	obs_data_release(encoder_settings);
...
obs_bridge_t *obs_bridge_create(const obs_bridge_config_t *config)
{
	...
	if (config) {
		...
		bridge->config.rtmp_horizontal_url = bstrdup("rtmp://localhost/live/obs_horizontal");
		bridge->config.rtmp_vertical_url = bstrdup("rtmp://localhost/live/obs_vertical");
...
/* Set configuration */
void obs_bridge_set_config(obs_bridge_t *bridge, const obs_bridge_config_t *config)
{
	if (!bridge || !config)
		return;
	...
	/* Copy new config */
	bridge->config.restreamer_url = config->restreamer_url ? bstrdup(config->restreamer_url) : NULL;
	bridge->config.rtmp_horizontal_url = config->rtmp_horizontal_url ? bstrdup(config->rtmp_horizontal_url) : NULL;
	bridge->config.rtmp_vertical_url = config->rtmp_vertical_url ? bstrdup(config->rtmp_vertical_url) : NULL;
	bridge->config.auto_start_enabled = config->auto_start_enabled;
	bridge->config.show_vertical_notification = config->show_vertical_notification;
...
void obs_bridge_get_config(obs_bridge_t *bridge, obs_bridge_config_t *config)
{
	...
	config->restreamer_url = bridge->config.restreamer_url ? bstrdup(bridge->config.restreamer_url) : NULL;
	config->rtmp_horizontal_url = bridge->config.rtmp_horizontal_url ? bstrdup(bridge->config.rtmp_horizontal_url) : NULL;
	config->rtmp_vertical_url = bridge->config.rtmp_vertical_url ? bstrdup(bridge->config.rtmp_vertical_url) : NULL;
	config->auto_start_enabled = bridge->config.auto_start_enabled;
	config->show_vertical_notification = bridge->config.show_vertical_notification;
...
/* Set profile manager */
void obs_bridge_set_profile_manager(obs_bridge_t *bridge, profile_manager_t *pm)
{
	if (bridge)
...
bool obs_bridge_start_horizontal(obs_bridge_t *bridge)
{
	if (!bridge || !bridge->config.rtmp_horizontal_url) {
		obs_log(LOG_ERROR, "[OBS Bridge] Cannot start horizontal: invalid configuration");
		return false;
	}

	/* Create encoder if needed */
	if (!bridge->horizontal_video_encoder) {
		bridge->horizontal_video_encoder = create_video_encoder("polyemesis_horizontal_video");
		if (!bridge->horizontal_video_encoder) {
			obs_log(LOG_ERROR, "[OBS Bridge] Failed to create horizontal video encoder");
			return false;
		}
	}

	if (!bridge->horizontal_audio_encoder) {
		bridge->horizontal_audio_encoder = create_audio_encoder("polyemesis_horizontal_audio");
		if (!bridge->horizontal_audio_encoder) {
			obs_log(LOG_ERROR, "[OBS Bridge] Failed to create horizontal audio encoder");
			return false;
		}
	}
	...
	if (!bridge->horizontal_output) {
		/* Create RTMP service */
		obs_service_t *service = create_rtmp_service(bridge->config.rtmp_horizontal_url);
		if (!service) {
			obs_log(LOG_ERROR, "[OBS Bridge] Failed to create RTMP service for horizontal");
			return false;
		}

		/* Create output */
		bridge->horizontal_output = obs_output_create("rtmp_output", "polyemesis_horizontal_output", NULL, NULL);
		if (!bridge->horizontal_output) {
		...
		obs_output_set_video_encoder(bridge->horizontal_output, bridge->horizontal_video_encoder);
		obs_output_set_audio_encoder(bridge->horizontal_output, bridge->horizontal_audio_encoder, 0);
	...
	if (!obs_output_start(bridge->horizontal_output)) {
		...
		obs_log(LOG_ERROR, "[OBS Bridge] Failed to start horizontal output: %s", error ? error : "unknown");
		return false;
	}

	obs_log(LOG_INFO, "[OBS Bridge] Horizontal output started -> %s", bridge->config.rtmp_horizontal_url);
	return true;
}
...
bool obs_bridge_start_vertical(obs_bridge_t *bridge)
{
	if (!bridge || !bridge->config.rtmp_vertical_url) {
		obs_log(LOG_WARNING, "[OBS Bridge] Cannot start vertical: invalid configuration");
		return false;
	}
	...
	if (!bridge->vertical_video_encoder) {
		bridge->vertical_video_encoder = create_video_encoder("polyemesis_vertical_video");
		if (!bridge->vertical_video_encoder) {
			obs_log(LOG_ERROR, "[OBS Bridge] Failed to create vertical video encoder");
			return false;
		}

		/* Connect to vertical canvas video */
		if (bridge->vertical_video) {
			obs_encoder_set_video(bridge->vertical_video_encoder, bridge->vertical_video);
		}
	}

	if (!bridge->vertical_audio_encoder) {
		bridge->vertical_audio_encoder = create_audio_encoder("polyemesis_vertical_audio");
		if (!bridge->vertical_audio_encoder) {
			obs_log(LOG_ERROR, "[OBS Bridge] Failed to create vertical audio encoder");
			return false;
		}
	}
	...
	/* Create output if needed */
	if (!bridge->vertical_output) {
		obs_service_t *service = create_rtmp_service(bridge->config.rtmp_vertical_url);
		if (!service) {
			obs_log(LOG_ERROR, "[OBS Bridge] Failed to create RTMP service for vertical");
			return false;
		}

		bridge->vertical_output = obs_output_create("rtmp_output", "polyemesis_vertical_output", NULL, NULL);
		if (!bridge->vertical_output) {
		...
		obs_output_set_video_encoder(bridge->vertical_output, bridge->vertical_video_encoder);
		obs_output_set_audio_encoder(bridge->vertical_output, bridge->vertical_audio_encoder, 0);
	...
	if (!obs_output_start(bridge->vertical_output)) {
		...
		obs_log(LOG_ERROR, "[OBS Bridge] Failed to start vertical output: %s", error ? error : "unknown");
		return false;
	}

	obs_log(LOG_INFO, "[OBS Bridge] Vertical output started -> %s", bridge->config.rtmp_vertical_url);
	return true;
}
...
void obs_bridge_handle_frontend_event(obs_bridge_t *bridge, enum obs_frontend_event event)
{
	if (!bridge)
		return;
	...
	case OBS_FRONTEND_EVENT_STREAMING_STARTING:
		...
		if (bridge->config.auto_start_enabled) {
			obs_log(LOG_INFO, "[OBS Bridge] Auto-start enabled, creating RTMP outputs");
