...
static void *restreamer_output_create(obs_data_t *settings, obs_output_t *output)
{
	struct restreamer_output *context =
		bzalloc(sizeof(struct restreamer_output));
	...
	if (context->is_multistream) {
		...
		restreamer_multistream_load_from_settings(
			context->multistream_config, settings);
	}
...
static bool restreamer_output_start(void *data)
{
	...
	if (!context->api) {
		obs_log(LOG_ERROR,
			"Cannot start output: API connection not initialized");
		return false;
	}
	...
	if (context->is_multistream && context->multistream_config) {
		...
		if (video) {
			...
			stream_orientation_t orientation =
				restreamer_multistream_detect_orientation(
					voi->width, voi->height);
			context->multistream_config->source_orientation =
				orientation;
		}

		bool result = restreamer_multistream_start(
			context->api, context->multistream_config,
			input_url.array);
		...
		if (!rtmp_url || !stream_key) {
			obs_log(LOG_ERROR,
				"No streaming service configured for output");
			return false;
		}
		...
		dstr_init(&reference);
		dstr_printf(&reference, "obs_output_%lld",
			    (long long)os_gettime_ns());

		bool result = restreamer_api_create_process(
			context->api, reference.array,
			"rtmp://localhost/live/obs_input", output_urls, 1, NULL);
...
static void restreamer_output_stop(void *data, uint64_t ts)
{
	...
	if (context->process_reference && context->api) {
		if (context->is_multistream) {
			restreamer_multistream_stop(context->api,
						    context->process_reference);
		} else {
			restreamer_api_stop_process(context->api,
						    context->process_reference);
		}
	}
...
static obs_properties_t *restreamer_output_properties(void *data)
{
	...
	obs_properties_add_bool(props, "enable_multistream",
				"Enable Multistreaming");

	obs_properties_add_bool(props, "auto_detect_orientation",
				"Auto-detect Video Orientation");

	obs_property_t *orientation = obs_properties_add_list(
		props, "source_orientation", "Force Orientation",
		OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

	obs_property_list_add_int(orientation, "Auto Detect",
				  ORIENTATION_AUTO);
