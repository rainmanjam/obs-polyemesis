...
PLUGIN_STATIC void *restreamer_source_create(obs_data_t *settings,
  ...
  if (use_global) {
    ...
    context->connection = restreamer_config_load_from_settings(settings);
    if (context->connection) {
      context->api = restreamer_api_create(context->connection);
    }
  }
    ...
    if (restreamer_api_get_processes(api, &list)) {
      for (size_t i = 0; i < list.count; i++) {
        const char *name = list.processes[i].reference
                               ...
                               : list.processes[i].id;
        obs_property_list_add_string(process_list, name, list.processes[i].id);
      }
...
PLUGIN_STATIC obs_properties_t *restreamer_source_get_properties(void *data) {
  ...
  obs_property_t *process_list =
      obs_properties_add_list(props, "process_id", "Restreamer Process",
                              OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);

  obs_property_list_add_string(process_list, "Select a process...", "");

  obs_properties_add_button(props, "refresh_processes", "Refresh Process List",
                            refresh_processes_clicked);

  /* Direct stream URL */
  obs_properties_add_text(props, "stream_url", "Or enter stream URL directly",
                          OBS_TEXT_DEFAULT);
